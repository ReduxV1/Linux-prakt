...
void close_file(){
    if (remove(temp_file) == 0) printf("Файл закрыт успешно.");
    else printf("Файл не закрыт.");
    getcharLock(2);
}
...
void edit_note(){
    ...
    printf("Новый ID вид дерева: ");
    scanf("%d", &tree_array[index].type);

    printf("Новый сорт ID: ");
    scanf("%d", &tree_array[index].sort);

    printf("Новый сезон посадки : ");
...
void export_txt(){
    ...
    fclose(out);
    printf("\nДанные экспортированны в 'out.txt'\n");
    printf("Нажмите ENTER чтобы вернуться назад...\n");
    getcharLock(2);
}
...
void about(){
    printf("Программа, осуществляет обработку данных из файла и запись результата обратно");
    printf("\nНажмите ENTER чтобы вернуться назад...\n");
    getcharLock(2);
}
